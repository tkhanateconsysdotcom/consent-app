...
export default ({
  ...
  const acceptanceValue = getInternetUsageCarbonImpact(
    sliderValue,
  ...
  return (
    <React.Fragment>
      <View style={styles.typeContainer}>
        <Text.H3>{t("ADD_PARTNER_SCREEN_STREAMING_TYPE")}</Text.H3>
      </View>
      ...
      <View style={styles.totalContainer}>
        <Text.H3 style={styles.miniHeader}>
          {t("ADD_PARTNER_SCREEN_TOTAL")}
        </Text.H3>
        <Text.H2 blue50>
          <FormattedNumber
            value={
              acceptanceValue > 1 ? acceptanceValue : acceptanceValue * 1000
            }
            maximumFractionDigits={2}
          />{" "}
          <Text.Primary>
            {acceptanceValue > 1 ? "kgCO2eq" : "gCO2eq"}
          </Text.Primary>
        </Text.H2>
      </View>
    </React.Fragment>
