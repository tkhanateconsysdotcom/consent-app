...
appJsonTemplate.expo.hooks.postPublish.find(
  (item) => item.file === "sentry-expo/upload-sourcemaps"
).config.authToken = process.env.SENTRY_AUTH_TOKEN;
...
fs.writeFile(`${path.join(pwd, "secret.ts")}`, secrets, () => {
